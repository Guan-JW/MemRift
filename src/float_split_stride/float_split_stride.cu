...
constexpr int KEEP_LIMIT = 1;                 // 每 bucket 至多缓存 8 块
...
std::vector<at::Tensor> pack_tensor(const at::Tensor& t, unsigned long long stream_ptr) {
    ...
    TORCH_CHECK(t.dim() <= 4, "split supports tensors with at most 4 dimensions");
    ...
    if (N == 0) {
        if (t.scalar_type() == at::kFloat)
            return {exp_host, at::empty({0, 3}, t.options().dtype(at::kByte))};
        return {exp_host, at::empty({0}, t.options().dtype(at::kByte))};
    }
    dim3 grid = grid_for(N);
    // auto exp_host = Pool::inst().get(N, at::kByte, -1, true, raw);
    int64_t sm_elems=(t.scalar_type()==at::kFloat)?N*3:N;
    // Pooled<false> sm(sm_elems, at::kByte, dev_idx, raw);
...
at::Tensor unpack_tensor(at::Tensor exp,
    ...
    TORCH_CHECK(exp.scalar_type() == at::kByte && sm.scalar_type() == at::kByte,
                "exp and sm must have uint8 dtype");
    TORCH_CHECK(dtype == at::kFloat || dtype == at::kBFloat16, "dtype mismatch");
    TORCH_CHECK(sizes.size() == strides.size(), "sizes and strides must have the same length");
    TORCH_CHECK(sizes.size() <= 4, "merge supports tensors with at most 4 dimensions");
    TORCH_CHECK(storage_offset >= 0, "storage_offset must be non-negative");

    int64_t expected_numel = 1;
    for (size_t i = 0; i < sizes.size(); ++i) {
        TORCH_CHECK(sizes[i] >= 0, "tensor sizes must be non-negative");
        TORCH_CHECK(strides[i] >= 0, "negative strides are not supported");
        if (sizes[i] == 0) {
            expected_numel = 0;
        } else if (expected_numel != 0) {
            TORCH_CHECK(expected_numel <= std::numeric_limits<int64_t>::max() / sizes[i],
                        "tensor shape overflows int64");
            expected_numel *= sizes[i];
        }
    }
    TORCH_CHECK(exp.numel() == expected_numel,
                "exp element count does not match output shape");
    TORCH_CHECK(dtype != at::kFloat || expected_numel <= std::numeric_limits<int64_t>::max() / 3,
                "sign/mantissa size overflows int64");
    const int64_t expected_sm = dtype == at::kFloat ? expected_numel * 3 : expected_numel;
    TORCH_CHECK(sm.numel() == expected_sm,
                "sm element count does not match output shape and dtype");
    ...
    if (expected_numel == 0) {
        ...
        auto flat = at::empty({need}, sm.options().dtype(dtype));
        return flat.as_strided(sizes, strides, storage_offset);
    }
