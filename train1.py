...
def get_cctx(algo="zstd", level=-1):
    try:
        ...
        if algo == "zstd":
            _tls.cctx = zstd.ZstdCompressor(level=level, threads=-1, write_checksum=False)
        elif algo == "lz4":
...
def build_hooks(rt: HookRuntime):
    def _pack(t: torch.Tensor):
        ...
        if new_key:
            ...
            rt.d2h_stream.wait_stream(torch.cuda.current_stream())
            with torch.cuda.stream(rt.d2h_stream):
                cpu_pin_buf, sm_bits = fs_sp.split(t, rt.d2h_stream.cuda_stream)
                evt = rt.d2h_stream.record_event()   # 拷贝结束事件
                t.record_stream(rt.d2h_stream)
            p2 = time.time()
            prep_time2 += p2 - p1   
            ...
            # ---- 3) 生成 token ----
            token = Token(cpu_pin_buf, sm_bits, evt, rid)
            token.st_ref = weakref.ref(t.untyped_storage())     # weakref to the original t
