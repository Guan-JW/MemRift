...
class AsyncCompressor:
    ...
    def decompress_sync(self, 
        ...
        # 此时 free 其实没有用，因为 压缩数据被 hold 在 graph 中，动弹不得
        # self.release_comped_data(tok)
        tok.decomped_data = rst
        return rst
    
    def release_comped_data(self,
            tok: PlaceHolderToken):
        del tok.comped_cpu_exp
        tok.comped_cpu_exp = None
        ...
        def _unpack(tok):
            if args.weight:
                ...
                if isinstance(tok, CompressedParam):
                    if args.debug:
                        print(f"[unpack]: layer {lid} is Compressed Param! {tok.data.shape=}")
                    # assert tok.source() is not None
                    # tok = tok.source()

                    # tok._ready_event.wait()
                    # tok.CtoD_evt.synchronize()
                    # tok.set_param()
                    # assert tok.data.numel() > 0
                    # return tok.data
            ...
            if isinstance(tok, PlaceHolderToken):
                if tok.decomped_data is None:
                    self.comp.decompress_sync(tok, torch.cuda.current_device())
                    # torch.cuda.empty_cache()
